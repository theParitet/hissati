...
import { ui, enumLabel, QUESTION_TEXT, toLocaleDigits } from "@/lib/i18n";
...
export default function Questionnaire() {
  ...
  function answerAndAdvance(patch: Partial<Profile>) {
    setAnswer(patch);
    // recompute steps after this answer (relocation may appear/disappear)
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-20 pt-8 sm:px-6">
      ...
      <div className="no-print">
        <div className="flex items-center justify-between text-sm text-ink-soft">
          ...
          <span className="font-medium text-oasis">
            {toLocaleDigits(matching, locale)}{" "}
            {matching === 1 ? t.stillMatchOne : t.stillMatch}
          </span>
        </div>
      ...
      {/* question */}
      <section className="mt-10" key={currentId}>
      ...
      <div className="no-print mt-10 flex items-center justify-between">
        ...
        {currentId === "funding" ? (
          <Button onClick={goNext} disabled={!fundingReady}>
            {clamped === steps.length - 1 ? t.seeResults : t.next}
            <Forward className="h-4 w-4" aria-hidden />
          </Button>
        ) : (
          isProfileComplete(answers) && (
